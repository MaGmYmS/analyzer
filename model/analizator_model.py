...
class Analyser:
    ...
    # def analyse_robot_movement(self):
    #     last_frames = self.frame_buffer.get_frames()[-5:]  # Получаем последние 5 кадров
    #     # Список для хранения позиций самого большого робота в каждом кадре
    #     largest_robot_positions = []
    #
    #     # Проходим по последним кадрам и находим самого большого робота
    #     for frame in last_frames:
    #         if frame.robot:  # Проверяем, есть ли роботы в кадре
    #             largest_robot = max(frame.robot, key=lambda r: r.width * r.high)  # Находим самого большого робота
    #             largest_robot_positions.append((largest_robot.x_center, largest_robot.y_center))
    #
    #     # Проверяем движение только для самого большого робота
    #     robot_in_motion = False
    #     print(largest_robot_positions)
    #     if len(largest_robot_positions) > 1:
    #         for i in range(1, len(largest_robot_positions)):
    #             # Вычисляем расстояние между позициями
    #             distance = np.sqrt((largest_robot_positions[i][0] - largest_robot_positions[i-1][0]) ** 2 +
    #                                (largest_robot_positions[i][1] - largest_robot_positions[i-1][1]) ** 2)
    #             # Устанавливаем порог для определения движения
    #             print(f"Смещение центра: {distance}")
    #
    #             if distance > 5:  # Задайте подходящее значение порога
    #                 robot_in_motion = True
    #                 break
    #
    #
    #     # Выводим статус движения робота
    #     if robot_in_motion:
    #         print("Робот в движении")
    #     else:
    #         print("Робот в ожидании")

    def analyse_robot_movement(self):
        last_frames = self.frame_buffer.get_frames()[-10:]  # Получаем последние 5 кадров
        # Список для хранения позиций самого большого робота в каждом кадре
        largest_robot_positions = []
        movement_results = []  # Массив для хранения результатов анализа
        ...
        if len(largest_robot_positions) > 1:
            for i in range(1, len(largest_robot_positions)):
                ...
                if distance > 5:  # Задайте подходящее значение порога
                    ...
                    break
        if robot_in_motion:
            print(f"Робот: в движении")
        else:
            print(f"Робот: в ожидании")


        # # Если количество результатов меньше 2, ничего не делаем
        # if len(movement_results) > 1:
        #     # Определяем наиболее частый результат
        #     most_common_result = Counter(movement_results).most_common(1)[0][0]
        # else:
        #     most_common_result = "в ожидании"  # Значение по умолчанию, если результатов нет

        # Выводим статус движения робота
        # print(movement_results)
        # print(f"Робот: {most_common_result}")
